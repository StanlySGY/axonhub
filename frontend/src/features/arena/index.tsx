...
import { Plus, Trash2, Send, Square, Menu } from 'lucide-react';
  ...
  return (
    <TooltipProvider>
      <div className="bg-background flex h-screen w-full">
        ...
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          ...
          <SheetContent side="left" className="w-[300px] p-0">
            <SheetHeader className="border-b p-4">
              <SheetTitle>{t('arena.title')}</SheetTitle>
              <p className="text-muted-foreground text-xs">{t('arena.description')}</p>
            </SheetHeader>
            {sidebarContent}
          </SheetContent>
        </Sheet>
        ...
        {/* Desktop Sidebar */}
        <div className="bg-card shadow-soft border-border m-4 hidden w-[300px] min-w-[260px] flex-col rounded-2xl border md:flex">
          ...
          <div className="shadow-soft border-border bg-card flex h-full flex-col rounded-2xl border">
            ...
            <div className="flex-1 overflow-hidden p-4">
              <PanelGroup
                ...
                className="h-full"
                autoSaveId="arena-panel-layout"
              >
                {panels.map((panel, index) => (
                  <Panel key={panel.id} minSize={20} defaultSize={100 / panels.length}>
                    {index > 0 && <PanelResizeHandle className="mx-2 w-1 rounded bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600" />}
                    <ArenaPanel
                      panel={panel}
